...
export function EmailErrorLogPanel({ failedCompanies }: { failedCompanies: EmailCompany[] }) {
        ...
        {failedCompanies.map((c) => (
          <div key={c.id} className="flex items-start gap-2 p-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">{c.error_message || "Unknown error"}</p>
            </div>
          </div>
        ))}
      </div>
